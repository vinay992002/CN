...
int main(){
   ...
   // unlink(userv_addr[i].sun_path);
	...
	printf("%d\n",portno[i]);
	serv_addr[i].sin_port = htons(port[i]);//htons  converts a port number in host byte order to a port number in network byte order
	...
	listen(uefd,5);
  	...
  	int unefd = accept(uefd,(struct sockadd*)&remote,&rlen);
  	fd_set readfd;
  	int maxfd;
  	struct timeval t;
  	t.tv_sec=1;
